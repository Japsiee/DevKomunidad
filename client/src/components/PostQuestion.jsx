...
const PostQuestion = ({ data }) => {
  ...
  return(
    ...
    { 
      data.map(dat => (
        <div className="PostEach bg-gray-50 border-t-4 border-purple text-gray-600 py-2 px-3 m-3 rounded shadow-sm relative" key={ dat._id }>
          {
          ...
          <p className="text-2xl font-semibold text-gray-700">{ dat.title }</p>
          <p className="text-xl">Asked by: <a href="/" className="text-purple-700 hover:text-purple-500">{ dat.from }</a></p>
          <span className="absolute top-2 right-5 font-semibold text-purple-500">{ dat.tag }</span>

          <p className="absolute top-7 right-5 font-semibold text-purple-500 text-lg">
          ...
          { 
            getQuestion(dat.question) ? 
              <p className="text-lg text-gray-500 my-4">{ dat.question.slice(0,250) }<button type="button" className="font-bold" onClick={ e => setUnfold(e, dat.question) } >&nbsp; . . . </button></p>
            :
